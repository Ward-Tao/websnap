		...
		init:function(){
			...
			Tasks.$txtTaskTitle.addEventListener('keyup',function(ev){
				...
				if(ev.keyCode==13){
					...
					sendMessageToContentScript({cmd:'test', value:'保存页面'}, function(response)
					{
						var url = response[0]
		...
		AppendHtml:function(task){
			...
			oDiv.addEventListener('click',function(){
				if(!task.is_finished){
					...
					Tasks.Edit(task);
					alert('跳转页面，再次点击回到上次浏览位置')
					sendMessageToContentScript({cmd:'test2', value:task}, function(response)
					{
					...
					if(confirm('是否删除快照？\r\n\r\n点击确定删除，点击取消置为未完成。')){
						...
						sendMessageToContentScript({cmd:'test3', value:task}, function(response)
						{
							console.log(response)
						});
					}else{
